...
function movenemies() {
  ...
  if(gametime%20==0)
    $("#score").text("score "+ gametime+5);
  else
  $("#score").text("score "+ gametime);

  if (gametime >= 0 && gametime < 100) speed = speedConstant;
  else if (gametime >= 100 &&  gametime < 200) speed = speedConstant-20;
  else if (gametime >= 200 &&  gametime < 300) speed = speedConstant-40;
  else if (gametime >= 300 &&  gametime < 400) speed = speedConstant-50;
  else if (gametime >= 400 &&  gametime < 500) speed = speedConstant-60;
  else speed = 10;
  ...
  {
    speedConstant=valueS;
  }
  console.log(speedConstant + " " + valueS + " " + a);
}
...
//      else document.location.reload();
      //document.location.reload();
    }                                       //  have tp change this value  socument.reload  //
}
function output () {
  window.alert('You survived ' + finaltime + ' seconds !');
  document.location.reload();
...
function checkLocation(e){
  ...
  $(document).keydown(function(evt) {
    ...
    if (evt.keyCode == 32&&space==false) {
      space = true;
     document.releaseEvents(Event.MOUSEMOVE,Event.MOUSEUP,Event.MOUSEUP);
      //setTimeout(movenemies,8000);
      //  speed=90000; 
      //gametime=100000;
     // speed=100000;
    //  clearTimeout();
 ...
 function resume () {
  ...
  $("#box").click(function(){
    console.log("  mouse event for resume")
        
        if(space==true){
        space=false;
        document.captureEvents(Event.MOUSEMOVE,Event.MOUSEUP,Event.MOUSEUP);
